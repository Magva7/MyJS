...
ajaxUtils.sendGetRequest = 
  function(requestUrl, responseHandler) {/**Это параметры, которые ожидает функция, т.е. которые мы внутрь нее
   ...
   var request = getRequestObject(); /**Это у нас объект, а не просто переменная, мы для этого объекта ниже свойсво
   onreadystatechange добавим, а сейчас в этой строке мы в него сохраняем то, что получили из функции выше, где
   ...
   новый объект XMLHttpRequest**/
    request.onreadystatechange = /**Т.е. мы для объекта request создаем свойство onreadystatechange, в котором у нас
     будет храниться то, что вернула функция handleResponse которая ниже, а она у нас возращает код ответа от сервера,
     т.е. например 200, что все ок, или 500, что внутренняя ошибка сервера. Т.е. тут храниться ответ от сервака на данный
     момент, т.е. это как бы состояние на текущий момент, т.е. разные этапы сетевой связи между браузером и
     сервером. Мы тут функцию не выполняем, а передаем просто значение этой функции, т.е. записываем в свойство саму
     функцию, а не то, что она возвращает**/
      function() { 
    ...
    request.open("GET", requestUrl, true); /**Здесь мы для нашего объекта request добавляем свойство open и в него
       записываем что-то, пока хз что. Это параметры,
       которые ожидает функция, т.е.
       которые мы внутрь нее
    ...
    request.send(null); /** for POST only - Тоже самое - создаем для нашего объекта request свойство send и
      // что-то в него записываем, пока хз, что
      // просто отправляем наш
      // запрос, т.е. все, что выше, это
      // настройка
